...
func TestHandleKaspiError(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			handlers.HandleKaspiError(recorder, tc.err, log)
...
func TestResponseHelpers(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			helperFunc:     handlers.BadRequestError,
			expectedStatus: http.StatusBadRequest,
		},
		{
			name:           "InternalServerError",
			helperFunc:     handlers.InternalServerError,
			expectedStatus: http.StatusInternalServerError,
		},
		{
			name:           "NotFoundError",
			helperFunc:     handlers.NotFoundError,
			expectedStatus: http.StatusNotFound,
		},
		{
			name:           "ConflictError",
			helperFunc:     handlers.ConflictError,
			expectedStatus: http.StatusConflict,
		},
		{
			name:           "ForbiddenError",
			helperFunc:     handlers.ForbiddenError,
			expectedStatus: http.StatusForbidden,
		},
		{
			name:           "ServiceUnavailableError",
			helperFunc:     handlers.ServiceUnavailableError,
			expectedStatus: http.StatusServiceUnavailable,
		},
		{
			name:           "UnauthorizedError",
			helperFunc:     handlers.UnauthorizedError,
...
func TestDecodeJSONRequest(t *testing.T) {
	...
	t.Run("successfully decodes valid JSON", func(t *testing.T) {
		...
		result := handlers.DecodeJSONRequest(recorder, req, &decoded)
